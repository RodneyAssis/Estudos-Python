...
class MyGame(arcade.Window):
    ...
    def __init__(self, width, height, title):
        ...
        #Sons
        self.complete_line = arcade.load_sound(":resources:sounds/fall4.wav")
        self.drop_ = arcade.load_sound(":resources:sounds/explosion2.wav")
        self.gameover_ = arcade.load_sound(":resources:sounds/gameover1.wav")
        "self.imggameover = arcade.load_texture_pair("")"
    ...
    def new_stone(self):
      ...
      """
        ...
        if check_collision(self.board, self.stone, (self.stone_x, self.stone_y)):
            arcade.play_sound(self.gameover_)
            self.game_over = True
            print("Gameover")
                ...
                while True:
                    for i, row in enumerate(self.board[:-1]):
                        if 0 not in row:
                            ...
                            print("Completando a linha")
                            arcade.play_sound(self.complete_line)
                            break
                    else:
                        print("Peça caiu")
                        arcade.play_sound(self.drop_)
                        break
                self.update_board()
                self.new_stone()
